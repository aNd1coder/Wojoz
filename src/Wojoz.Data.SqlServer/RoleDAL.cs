...
namespace Wojoz.Data.SqlServer
{
    ...
    {  
           ...
           using (DbConnection conn = db.CreateConnection())
			{
				...
				{ 
					try
					{ 
						using (DbCommand cmd = db.GetStoredProcCommand("SP_Role_Add"))
						{
                        ...
                        using (DbCommand cmd = db.GetStoredProcCommand("SP_DeleteRecord"))
                        {
                            ...
                            for (int i = 0; i < KeyValues.Length; i++)
                            {
                                if (KeyValues[i] != "")
                                {
                                    ...
                                    db.AddInParameter(cmd, "@TableName", DbType.String, "Role");
        ...
        public RoleInfo Get(int keyValue)
        {
            RoleInfo model = null;
        ...
        public IList<RoleInfo> Find(string tbFields, string strWhere, string orderField, int pageIndex, int pageSize, out int totalCount)
        {
			IList<RoleInfo> list = new List<RoleInfo>();
			using (DbCommand cmd = db.GetStoredProcCommand("SP_SqlPagenation"))
			{
				db.AddInParameter(cmd, "@TbName", DbType.String, "Role");

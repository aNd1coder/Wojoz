...
namespace Wojoz.Utilities
{
    public class HttpHelper
    {
        ...
        static public string GetSubString(string doc, string startTag, string endTag)
        {
            int start = 0,
                end = 0,
                length = 0;
            ...
            start = doc.IndexOf(startTag) + startTag.Length;
            if (start < startTag.Length)
            {
                return string.Empty;
            }
            end = doc.IndexOf(endTag, start);
            if (end == -1)
            {
                return string.Empty;
            }
            length = end - start;
            return doc.Substring(start, length);
        }

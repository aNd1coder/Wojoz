...
namespace Wojoz.Utilities
{
         ...
         new string[][]
   {
    ...
    new string[]{"Dan","但单蛋担弹掸胆淡丹耽旦氮诞郸惮疸澹瘅萏殚眈聃箪赕儋啖丼亶伔倓僤儃冄冉刐勯匰単呾唌啗啿單嘾噉噡嚪坛壇妉娊媅帎弾彈忱怛惔愖憚憺憾抌撢撣擔柦檐欿殫沊泹湛潭澶澸燀狚玬瓭甔疍癉癚皽砃禫窞簞紞繵耼聸腅膻膽蜑蜒蟺衴褝襌襜覘觇觛訑詹誕譂贉贍赡蹛躭鄲酖醈霮頕餤饏馾駳髧鴠黕黮黵卩亻"},
    ...
    new string[]{"Gan","赶干感敢竿甘肝柑杆赣秆旰酐矸疳泔苷擀绀橄澉淦尴坩乹乾亁仠佄倝凎凲咁奸尲尶尷幹忓扞捍攼桿榦檊汗汵浛漧灨玕玵皯盰稈笴筸篢簳粓紺芉虷衦詌諴豃贑贛趕迀釬錎飦骭魐鰔鱤鳡鳱"},
    ...
    new string[]{"Miu","谬缪繆謬"},
    new string[]{"Mo","摸磨抹末膜墨没莫默魔模摩摹漠陌蘑脉沫万无冒寞秣瘼殁镆嫫谟蓦貊貘麽茉馍耱么伯佰劘劰勿嗼嚤嚩圽塻妺嫼尛帓帞怽懡戂抚撫擵攠昧昩暯枺橅歾歿沒瀎無爅狢百皌眜眽眿瞐瞙砞礳粖糢絈絔縸纆艒莈藐藦蛨蟆蟔袜袹謨謩譕貃貈貉貌銆鏌靺鞨饃饝驀髍魩魹麼麿黙嘿嬷"},
    ...
    new string[]{"Zuo","做作坐左座昨凿琢撮佐笮酢唑祚胙怍阼柞乍侳咗岝岞挫捽柮椊砟秨稓筰糳繓苲莋葃葄蓙袏諎醋鈼鑿飵嘬阝"}
        ...
        /// <param name="toaddr">收件人地址</param>
        /// <param name="title">邮件标题</param>
        /// <param name="body">邮件内容</param>
        public static void SendMail(string server, string username, string password, string toaddr, string title, string body)
        {
            try
            {
                ...
                System.Net.Mail.MailMessage message = new MailMessage(username, toaddr, title, body);
        ...
        ///去除HTML标记
        ///</summary>
        ///<param name="value">包括HTML的源码   </param>
        ...
        public static string NoHTML(string value)
        {
            ...
            value = Regex.Replace(value, @"<!--.*", "", RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(quot|#34);", "\"",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(amp|#38);", "&",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(lt|#60);", "<",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(gt|#62);", ">",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(nbsp|#160);", "   ",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(iexcl|#161);", "\xa1",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(cent|#162);", "\xa2",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(pound|#163);", "\xa3",
              RegexOptions.IgnoreCase);
            value = Regex.Replace(value, @"&(copy|#169);", "\xa9",
              RegexOptions.IgnoreCase);
        ...
        ///</summary>
        ///<param name="value">value</param>
        public static string GetImgUrl(string value)
        {
            string str = string.Empty;
            Regex r = new Regex(@"<img\s+[^>]*\s*src\s*=\s*([']?)(?<url>\S+)'?[^>]*>",
              RegexOptions.Compiled);
            Match m = r.Match(value.ToLower());
            if (m.Success)
                str = m.Result("${url}");
            return str;
        }

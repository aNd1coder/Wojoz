...
namespace Wojoz.Cache
{
    ...
    public class ShopCache
    {
        ...
        private static volatile ShopCache instance = null;
        ...
        /// </summary>
        /// <returns></returns>
        public static ShopCache GetCacheService()
        {
            ...
            if (instance == null)
            {
                lock (lockHelper)
                {
                    if (instance == null)
                    {
                        instance = applyMemCached ? new ShopCache() : CachesFileMonitor.CheckAndRemoveCache(new ShopCache());
                    }
                }
            }
        ...
        public virtual void AddObject(string xpath,  SortedList<int, object> __sortedlist)
        {
            lock (lockHelper)
            {
                if (!applyMemCached)
        ...
        ///// <param name="xpath">分级对象的路径</param>
        ///// <param name="dt">缓存的数据</param>
        //public virtual void AddObject(string xpath, IList<CategoryInfo> catList)
        //{
        //    lock (lockHelper)
        //    {
        //        if (!applyMemCached)
        ...
        //            }
        //        }
        //        AddObject(xpath, (object)catList);
        //    }
        //}
            ...
            try
            {
                ...
                if (!applyMemCached && cacheObject != null)
                {
                    ...
                    if (cf == CacheFlag.CacheHaveData)
                    {
                        ...
                        {
                            return ((( SortedList<int, string>)cacheObject).Count == 0) ? null : cacheObject;
                        }
    ...
    public class CachesFileMonitor
    {
        ...
        private static string path = UrlHelper.GetMapPath("/cache/cache.config");
        ...
        static CachesFileMonitor()
        {
            if (!File.Exists(path))
            {
                ...
                content += "<cachetableremove>\r\n<table1 xpath=\"example\" removedatetime=\"2009-1-8 17:36:23\" />\r\n</cachetableremove>";
        ...
        /// <returns></returns>
        public static ShopCache CheckAndRemoveCache(ShopCache instance)    //
        {
